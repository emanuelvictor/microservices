...
public abstract class AbstractService<T, ID extends Serializable> {
     ...
     */
    @Autowired(required = false) // TODO couple, must use inject from jakarta
    private JpaRepository<T, UniqueIdentifier<ID>> typedIdentifierRepository;
     ...
     * @param id {@link ID}
     */
    public Optional<T> findById(final UniqueIdentifier<ID> id) {
     ...
     * @param id {@link ID}
     */
    public void deleteById(final UniqueIdentifier<ID> id) {
