...
//
//        alternatives = new int[]{7, 6, 1, 2, 3, 17, 21, 8};
...
//                pivots = subArrayIncludeRange(rotate(originals), 0, 0);
//                range = rotate(originals);
//                originals = rotate(originals);
//            } else {
//                pivots = subArrayIncludeRange(originals, 0, pivots.length);
//                range = subArrayIncludeRange(range, 1, range.length - 1);
//            }
//
//        } while (result.stream().distinct().count() < totalOfCombinations);
//
//        return result;
//    }
...
//    }
    ...
    @Test
    void generateTest() {
        final Set<Combination> combinations = generate(5);
        combinations.forEach(System.out::println);

        Assertions.assertThat(combinations.size()).isEqualTo(comb(5));
        Assertions.assertThat(generate(8).size()).isEqualTo(comb(8));
    }

    public Set<Combination> generate(int n) {
        final Set<Combination> elements = new HashSet<>();
        for (int i = 1; i <= n; i++) {
    ...
    public Set<Combination> generate(final int n, final int r) {
        final Set<Combination> combinations = new HashSet<>();
        ...
        while (combination[r - 1] < n) {
            combinations.add(new Combination(combination.clone()));
